...
export const getByCRMValidation = validation(getSchema => ({
    params: getSchema<IparamProps>(yup.object().shape({
        crm: yup.number().integer().required().moreThan(0),
    })),
}));

export const getByCRM = async (req: Request<IparamProps>, res: Response) => {
   ...
   const result = await MedicoProvider.getByCRM(req.params.crm);
