...
export const getByPendentValidation = validation(getSchema => ({
    params: getSchema<IparamProps>(yup.object().shape({
        status: yup.string().required(),
    })),
}));

export const getAllPendent = async (req: Request<IparamProps>, res: Response) => {
   ...
   const result = await ExamesProvider.getAllPendent(req.params.status);
